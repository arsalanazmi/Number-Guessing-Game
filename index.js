...
const range = await inquirer.prompt([
    {
        name: "min",
        type: "number",
        message: "Please Enter min-range for computer to randomly select number from ?",
    },
    {
        name: "max",
        type: "number",
        message: "Please Enter max-range for computer to randomly select number from ?",
    },
]);
...
if (answer.userGuessedNumber > range.max ||
    answer.userGuessedNumber < range.min) {
    console.log("Your guessed is outside the range you defined. Please try again!");
...
else if (randomNumber === answer.userGuessedNumber + 1 ||
